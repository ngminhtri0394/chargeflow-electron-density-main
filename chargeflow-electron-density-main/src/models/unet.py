...
class Upsample(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.dims == 3:
            # For 3D, calculate target size more carefully to handle odd dimensions
            target_size = (
                x.shape[2] * 2,  # Keep first spatial dim unchanged
                x.shape[3] * 2,  # Double second spatial dim
                x.shape[4] * 2   # Double third spatial dim
            )
...
class Downsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None):
        ...
        self.use_conv = use_conv
        self.dims = dims
        stride = 2 if dims != 3 else (2, 2, 2)  # For 3D, only downsample last two dimensions
        if use_conv:
            self.op = conv_nd(
...
class UNetModel(nn.Module):
    ...
    input_downscale: int = 4  # Downscale input by this factor
    ...
    def __post_init__(self):
            ...
            self.input_blocks = nn.ModuleList(
                [
                    TimestepEmbedSequential(
                        conv_nd(self.dims, self.in_channels, ch, 3, stride=1, padding='same', padding_mode='circular')
                    )
                ]
            )
        else:
        ...
        ds = 1
        self.input_downscale_blocks = nn.ModuleList([])
        for i in range(self.input_downscale//2):
            self.input_downscale_blocks.append(Downsample(
                self.in_channels, use_conv=True, dims=self.dims, out_channels=self.in_channels
            ))
        ...
        for level, mult in list(enumerate(self.channel_mult))[::-1]:
            for i in range(self.num_res_blocks + 1):
                ...
                self._feature_size += ch
        self.output_upscale_blocks = nn.ModuleList([])
        for i in range(self.input_downscale//2):
            self.output_upscale_blocks.append(Upsample(
                1, use_conv=True, dims=self.dims, out_channels=1
            ))
    ...
    def forward(self, x, timesteps, extra):
        ...
        h = x
        # print('extra:', extra)
        if "concat_conditioning" in extra and extra["concat_conditioning"] is not None:
            h = torch.cat([x, extra["concat_conditioning"]], dim=1)
        
        for module in self.input_downscale_blocks:
            h = module(h)
        ...
        for module in self.output_blocks:
            skip_h = hs.pop()
            # For 3D data, ensure spatial dimensions match before concatenation
            if self.dims == 3 and h.shape[2:] != skip_h.shape[2:]:
                # Upsample h to match skip_h's spatial dimensions
                h = F.interpolate(h, size=skip_h.shape[2:], mode="nearest")
            h = torch.cat([h, skip_h], dim=1)
            h = module(h, emb)
        h = h.type(x.dtype)
        result = self.out(h)
        for module in self.output_upscale_blocks:
            result = module(result)
        # result = result / torch.sum(result, axis=(-3,-2,-1))[...,None,None,None]
        # result = result * torch.sum(extra["label"], axis=(-3,-2,-1))[...,None,None,None]
        return result
